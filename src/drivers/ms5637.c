...
#define MS5637_COEFFICIENT_NUMBERS									7  // or 7 or 8?
...
static uint16_t eeprom_coeff[MS5637_COEFFICIENT_NUMBERS + 1] = {0};
...
enum ms5637_status ms5637_read_eeprom(void)
{
	...
	{
		status = ms5637_read_eeprom_coeff( (uint8_t)(MS5637_PROM_ADDRESS_READ_ADDRESS_0 + i*2), eeprom_coeff+i);
		if(status != ms5637_status_ok)
			return status;
	}

	if( !ms5637_crc_check( eeprom_coeff, (uint8_t)((eeprom_coeff[MS5637_CRC_INDEX] & MS5637_CRC_MASK) >> MS5637_CRC_SHIFT )) )
...
enum ms5637_status ms5637_read_temperature_and_pressure( float *temperature, float *pressure)
{
	...
	cmd = (uint8_t)(ms5637_resolution_osr*2);
	...
	TEMP = 2000 + (int32_t)((int64_t)dT * (int64_t)eeprom_coeff[MS5637_TEMP_COEFF_OF_TEMPERATURE_INDEX] >> 23) ;
	...
	*temperature = ( (float)TEMP - (float)T2 ) / 100;
...
bool ms5637_crc_check (uint16_t *n_prom, uint8_t crc)
{
    ...
    n_rem = 0x00;
    crc_read = n_prom[MS5637_CRC_INDEX];
    n_prom[MS5637_CRC_INDEX] = (uint16_t)(~MS5637_CRC_MASK & (n_prom[MS5637_CRC_INDEX]));
    for (cnt = 0; cnt < ((MS5637_COEFFICIENT_NUMBERS + 1) * 2); cnt++)
    {
        if (cnt%2==1) n_rem = (uint16_t)(n_rem ^ ((uint16_t)(n_prom[cnt>>1]) & 0x00FF));
    ...
    n_prom[MS5637_CRC_INDEX] = crc_read;
    n_rem ^= 0x00;
